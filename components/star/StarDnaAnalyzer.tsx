...
async function analyzeVisualAudio(blob: Blob): Promise<DnaVisualData> {
...
export default function StarDnaAnalyzer({ audio, title, grade, score, verified = true }: Props) {
  ...
  useEffect(() => {
    ...
    void analyzeVisualAudio(audio)
      .then((next) => {
        if (cancelled) return;
        setData(next);
        setSelectedIndex(Math.floor(next.frames.length / 2));
      })
      .catch((caught) => {
  ...
  function inspect(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    setSelectedIndex(Math.round(ratio * (data.frames.length - 1)));
  }
      ...
      <div className="border-b border-white/8 bg-gradient-to-r from-sky-400/[0.08] via-transparent to-violet-400/[0.06] p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[9px] font-black uppercase tracking-[.24em] text-sky-300">CrucibleStar · Audio DNA</p>
            <h2 className="mt-1 truncate text-xl font-black text-white">{title}</h2>
            <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px] font-bold text-white/40">
          ...
          <div className="flex items-center gap-2">
            {verified ? <span className="inline-flex items-center gap-1 rounded-full border border-emerald-300/20 bg-emerald-400/10 px-3 py-1.5 text-[10px] font-black uppercase text-emerald-200"><ShieldCheck size={13} />Verified</span> : null}
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex rounded-xl border border-white/10 bg-black/25 p-1">
            <button type="button" onClick={() => setView("wave")} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-[10px] font-black uppercase ${view === "wave" ? "bg-white text-black" : "text-white/45"}`}><Waves size={14} />DNA Wave</button>
            <button type="button" onClick={() => setView("stack")} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-[10px] font-black uppercase ${view === "stack" ? "bg-white text-black" : "text-white/45"}`}><Layers3 size={14} />Analyzer Stack</button>
          </div>
          <span className="text-[9px] font-black uppercase tracking-wider text-white/30">Analysis only · no audio editing</span>
        </div>
      </div>
      ...
      <div className="p-3 sm:p-5">
        ...
        {view === "wave" ? (
          <div onPointerDown={inspect} onPointerMove={inspect} className="mt-2 touch-none overflow-hidden rounded-2xl border border-white/10 bg-black/45">
            <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="block h-72 w-full" preserveAspectRatio="none" role="img" aria-label="Interactive CrucibleStar DNA waveform">
              ...
              <path d={wave} fill={`${AUDIO_PARAMETER_COLORS.waveform}28`} stroke={AUDIO_PARAMETER_COLORS.waveform} strokeWidth="1.6" vectorEffect="non-scaling-stroke" />
        ...
        {selected ? (
          <div className="mt-3 grid gap-2 rounded-2xl border border-white/8 bg-white/[0.025] p-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="sm:col-span-2 lg:col-span-1"><p className="text-[9px] font-black uppercase tracking-wider text-sky-300">Inspection point</p><p className="mt-1 font-mono text-2xl font-black">{formatTime(selected.time)}</p></div>
            ...
            <div className="sm:col-span-2">
              <p className="text-[9px] font-black uppercase tracking-wider" style={{ color: AUDIO_PARAMETER_COLORS.eq }}>Frequency energy</p>
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-white/55">{selected.bands.map((value, index) => <span key={BAND_LABELS[index]}>{BAND_LABELS[index]} {value.toFixed(1)} dB</span>)}</div>
            </div>
          </div>
        ) : null}

        <div className="mt-3 rounded-xl border border-violet-300/10 bg-violet-400/[0.035] px-3 py-2 text-[10px] leading-5 text-white/38">
          Reverb and delay overlays are intentionally not fabricated. They will appear only when CrucibleStar has a measured decay/repeat model or exact Crucible-applied effect metadata for that version.
        </div>
      </div>
    </section>
  );
...
function Readout({ label, value, color }: { label: string; value: string; color: string }) {
  return <div className="rounded-xl bg-black/30 p-3"><p className="text-[9px] font-black uppercase tracking-wider" style={{ color }}>{label}</p><p className="mt-1 font-mono text-sm font-black text-white/85">{value}</p></div>;
}
