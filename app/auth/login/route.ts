...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!email || !password) {
      return NextResponse.json({ error: "missing-credentials" }, { status: 400, headers: { "Cache-Control": "private, no-store" } });
    }
    ...
    const { data: guardData, error: guardError } = await supabase.rpc("consume_login_attempt", {
    ...
    if (guard && guard.allowed === false) {
      return NextResponse.json(
        ...
        { status: 429, headers: { "Cache-Control": "private, no-store", "Retry-After": String(guard.retry_after_seconds ?? 900) } },
      );
    }
    ...
    const staleCookieNames = new Set(
      request.cookies.getAll().map(({ name }) => name).filter((name) => name.startsWith("sb-") || name.includes("auth-token")),
    );
    ...
    cookiesToSet.forEach(({ name, value, options }) => {
      appendCookie(response, name, value, domain ? { ...options, domain } : options);
    });
