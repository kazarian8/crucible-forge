...
type Moment = { id:string; body:string; music_url:string|null; artwork_url:string|null; created_at:string; username:string|null };
...
export default function MomentsPage(){
  ...
  const load = async () => {
    try {
      ...
      if (!response.ok) throw new Error("Unable to load Moments. Please try again.");
      const data = await response.json() as { moments: Moment[] };
      setMoments(data.moments);
    } catch (error) {
      setMsg(error instanceof Error ? error.message : "Unable to load Moments.");
    }
  };
  useEffect(()=>{void load();},[]);
  ...
  <section className="mt-6 space-y-3">{moments.length===0?<div className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-sm text-white/35"><Radio className="mx-auto mb-2"/>No public Moments yet.</div>:moments.map(m=><article key={m.id} className="rounded-2xl border border-white/10 bg-[#0d0a08] p-4"><header className="min-w-0"><p className="break-words text-sm font-semibold text-orange-200">{m.username ? `@${m.username.replace(/^@/, "")}` : "Artist unavailable"}</p><time dateTime={m.created_at} className="mt-1 block text-xs text-white/60">{new Date(m.created_at).toLocaleString(undefined, { year: "numeric", month: "short", day: "numeric", hour: "numeric", minute: "2-digit", timeZoneName: "short" })}</time></header><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-white/80">{m.body}</p>{m.artwork_url?<div className="mt-4 aspect-square overflow-hidden rounded-2xl bg-black bg-contain bg-center bg-no-repeat" style={{backgroundImage:`url(${m.artwork_url})`}}/>:null}{m.music_url?<audio className="mt-4 w-full" controls preload="none" src={m.music_url}/>:null}</article>)}</section></div></main>;
}
