...
export default async function ChallengePage() {
  ...
  if (user) {
    const { data: ownEntry } = await supabase.from("challenge_entries").select("id").eq("user_id", user.id).maybeSingle();
  ...
  const entries = ((leaderboardData ?? []) as LeaderboardRow[]).map((entry) => ({
    ...
    originality_percent: entry.originality_percent == null ? null : Number(entry.originality_percent),
    ...
    <main className="min-h-screen bg-black text-white">
      <section className="relative overflow-hidden border-b border-white/10 px-5 py-20">
        ...
        <div className="relative mx-auto max-w-6xl">
          ...
          <h1 className="mt-4 max-w-4xl text-5xl font-black leading-[0.95] sm:text-7xl">Talent is the currency.</h1>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-zinc-400">One verse. Your authentic voice. Free entry. Free voting. No vote bundles, paid boosts, or pay-to-win placement.</p>
          <div className="mt-8 flex flex-wrap gap-3 text-sm font-black uppercase tracking-[0.14em]">
            <span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-2 text-orange-300">One Verse Only</span>
            <span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-2 text-orange-300">Authentic Voice Only</span>
            <span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-2 text-orange-300">Free Entry</span>
            <span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-2 text-orange-300">One Free Vote</span>
          </div>
        </div>
      </section>
