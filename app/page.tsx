  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-black text-white">
            ...
            <div className="relative z-30 flex min-h-screen items-end px-5 pb-8 pt-28 sm:items-center sm:pb-12 md:px-8">
              <div className="w-full max-w-3xl rounded-[2rem] border border-orange-400/30 bg-black/82 p-6 shadow-[0_30px_100px_rgba(0,0,0,.85)] backdrop-blur-xl sm:p-8 md:p-10">
                <div className="inline-flex rounded-full border border-orange-300/25 bg-orange-400/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-orange-200 sm:text-xs">
                  Premium Access + Monthly Credits
                </div>

                <h1 className="mt-6 text-4xl font-black leading-[1.02] tracking-[-0.05em] text-white sm:text-5xl md:text-7xl">
                  Start creating today for $0.
                </h1>

                <p className="mt-5 max-w-2xl text-sm leading-7 text-white/72 sm:text-base">
                  Unlock the Crucible Furnace with a free 30-day Premium trial,
                  5,000 starter credits, and immediate access to every active
                  module. Membership keeps the platform unlocked. Credits power
                  each Furnace operation.
                </p>

                <div className="mt-7 grid gap-4 rounded-3xl border border-orange-400/25 bg-orange-500/[0.06] p-5 sm:grid-cols-3">
                  <div>
                    <p className="text-2xl font-black text-white">$0</p>
                    <p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-orange-300">Today</p>
                  </div>
                  <div>
                    <p className="text-2xl font-black text-white">5,000</p>
                    <p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-orange-300">Trial Credits</p>
                  </div>
                  <div>
                    <p className="text-2xl font-black text-white">$19.99</p>
                    <p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-orange-300">Monthly After Trial</p>
                  </div>
                </div>
                ...
                <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                  <a
                    ...
                    className="flex min-h-16 flex-1 items-center justify-center rounded-2xl bg-gradient-to-r from-orange-600 to-amber-500 px-6 py-4 text-center text-base font-black text-black shadow-[0_16px_45px_rgba(249,115,22,.25)] transition hover:brightness-110"
                  >
                  ...
                  <a
                    href="#furnace"
                    className="flex min-h-16 items-center justify-center rounded-2xl border border-white/20 bg-white/[0.05] px-7 py-4 text-center text-sm font-bold text-white transition hover:border-orange-400/60 hover:text-orange-200"
                  >
                ...
                <p className="mt-4 text-xs leading-5 text-white/55">
                  $0 due today. A valid payment method is required. Your free
                  30-day Premium trial includes 5,000 trial credits and access to
                  all currently active Crucible Furnace modules. Unless canceled
                  before the trial ends, your membership automatically renews at
                  $19.99 per month until canceled. Credit usage varies by module
                  and operation. Additional credits may be purchased separately.
                </p>

                <p className="mt-3 text-[11px] leading-5 text-white/40">
                  By selecting “Start Your Free Month,” you agree to the
                  <a href="/terms" className="mx-1 text-orange-300 hover:text-orange-200">Terms of Service</a>,
                  <a href="/privacy" className="mx-1 text-orange-300 hover:text-orange-200">Privacy Policy</a>,
                  <a href="/subscription-policy" className="mx-1 text-orange-300 hover:text-orange-200">Subscription and Cancellation Policy</a>, and
                  <a href="/credit-policy" className="ml-1 text-orange-300 hover:text-orange-200">Credit and Refund Policy</a>.
                </p>

                <div className="mt-6 grid gap-3 border-t border-white/10 pt-5 sm:grid-cols-3">
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-orange-300">Premium</p>
                    <p className="mt-1 text-xs leading-5 text-white/45">Membership unlocks the Furnace.</p>
                  </div>
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-orange-300">Credits</p>
                    <p className="mt-1 text-xs leading-5 text-white/45">Credits power each operation.</p>
                  </div>
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.18em] text-orange-300">Private</p>
                    <p className="mt-1 text-xs leading-5 text-white/45">No behavioral ad tracking.</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <a
        ...
        aria-label="Distribute your music with DistroKid"
        className="fixed right-0 top-1/2 z-50 flex -translate-y-1/2 items-center gap-3 rounded-l-2xl border border-r-0 border-orange-400/40 bg-black/90 px-3 py-4 shadow-[0_16px_45px_rgba(0,0,0,.65)] backdrop-blur-xl transition hover:bg-orange-500 hover:text-black sm:px-4"
      >
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-orange-500 text-lg font-black text-black">
          D
        </span>

        <span className="hidden pr-1 sm:block">
          <span className="block text-[10px] font-black uppercase tracking-[0.18em] text-orange-300 group-hover:text-black">
            Release Your Music
          </span>
