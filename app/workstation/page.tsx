...
export default function WorkstationPage() {
  ...
  return (
    <main className="min-h-screen bg-[#04070d] text-white">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0a1420]/95 backdrop-blur-xl">
        <div className="flex min-h-14 items-center justify-between gap-3 px-3 sm:px-5">
          ...
          <div className="flex items-center gap-1.5">
            {mixName ? <span className="hidden max-w-52 truncate text-xs font-bold text-emerald-300 lg:inline">{mixName} ready</span> : null}
            <Link href="/star" className="rounded-lg bg-orange-500 px-3 py-2 text-xs font-black text-black">CrucibleStar</Link>
            <Link href="/sound-furnace" className="rounded-lg border border-white/10 px-3 py-2 text-xs font-black text-white/65">Mastering</Link>
          </div>
        </div>
      </header>
