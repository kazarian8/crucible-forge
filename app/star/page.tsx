...
export default function CrucibleStarPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(`Saved privately as ${detectedCategory}. Detected ${analysis.contentType} · ${analysis.contentConfidence}% confidence · Technical status: ${analysis.status}. Sound-quality grade not calibrated. Publishing is optional.`);
      setAudioFile(null);
      setArtworkFile(null);
      setArtworkPreview("");
      setArtworkPrompt("");
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-7">
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          ...
          <div className="rounded-3xl border border-white/10 bg-[#0a111c] p-5">
            ...
            {lastAnalysis ? <div className="mt-4"><div className="flex items-end gap-3"><span className="text-lg font-black text-orange-300">Technical status: {lastAnalysis.status}</span></div><QualityReport quality={lastAnalysis.quality} /><div className="mt-3 flex flex-wrap gap-2">{lastAnalysis.contentTags.map((tag) => <span key={tag} className="rounded-full border border-orange-300/20 bg-orange-500/10 px-2.5 py-1 text-[10px] font-black uppercase text-orange-200">{tag}</span>)}</div><div className="mt-4 grid grid-cols-2 gap-2 text-xs"><Metric label="Detected type" value={`${lastAnalysis.contentType} · ${lastAnalysis.contentConfidence}%`} /><Metric label="Suggested category" value={lastAnalysis.suggestedCategory} /><Metric label="Estimated tempo" value={lastAnalysis.estimatedBpm ? `${lastAnalysis.estimatedBpm} BPM · ${lastAnalysis.bpmConfidence}%` : "No confident tempo"} /><Metric label="Estimated key" value={lastAnalysis.estimatedKey ? `${lastAnalysis.estimatedKey} · ${lastAnalysis.keyConfidence}%` : "No confident key"} /><Metric label="Peak" value={`${lastAnalysis.peakDb.toFixed(1)} dBFS`} /><Metric label="RMS" value={`${lastAnalysis.rmsDb.toFixed(1)} dBFS`} /><Metric label="Sample rate" value={`${lastAnalysis.sampleRate} Hz`} /><Metric label="Channels" value={String(lastAnalysis.channels)} /><Metric label="Duration" value={`${lastAnalysis.duration.toFixed(2)} sec`} /><Metric label="Silence" value={`${lastAnalysis.silencePercent.toFixed(1)}%`} /></div><div className="mt-4 space-y-2">{lastAnalysis.notes.map((note) => <p key={note} className="flex gap-2 text-xs leading-5 text-white/50"><CheckCircle2 size={14} className="mt-0.5 shrink-0 text-orange-300" />{note}</p>)}</div></div> : <div className="mt-8 text-center text-sm text-white/30"><Music2 className="mx-auto mb-3" />File DNA will identify the likely content, type, tempo, and key after you upload.</div>}
          </div>
        </section>
          ...
          <div className="mt-4 space-y-3">
            {files.map((item) => (
              <article key={item.id} className="rounded-2xl border border-white/8 bg-black/20 p-4">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black">{item.title}</h3><p className="mt-1 text-xs text-white/35">{item.category}{item.bpm ? ` · ${item.bpm} BPM` : ""}{item.musical_key ? ` · ${item.musical_key}` : ""}{item.price_cents > 0 ? ` · $${(item.price_cents / 100).toFixed(2)}` : " · free"}</p></div><div className="flex items-center gap-2"><span className="rounded-full border border-white/10 px-2 py-1 text-[10px] font-black uppercase text-white/55">{item.verification_status}</span><span className="rounded-xl bg-orange-500 px-3 py-1 text-sm font-black text-black">Technical checklist</span></div></div>
                {openDnaId === item.id ? <div>{item.analysis?.quality?.version === "quality-measurements-v1" ? <QualityReport quality={item.analysis.quality} comparison={item.analysis.quality_comparison} /> : <p className="mt-2 text-xs text-white/50">Legacy technical assessment. Reanalyze the audio for detailed quality checks.</p>}</div> : null}
